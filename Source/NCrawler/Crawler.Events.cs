...
namespace NCrawler
{
	public partial class Crawler
	{
		...
		{
			long downloadErrors = Interlocked.Increment(ref _downloadErrors);
			if (MaximumHttpDownloadErrors.HasValue && MaximumHttpDownloadErrors.Value > downloadErrors)
			{
				Logger.Error("Number of maximum failed downloads exceeded({0}), cancelling crawl", MaximumHttpDownloadErrors.Value);
				StopCrawl();
			}

			Logger.Error("Download exception while downloading {0}, error was {1}", crawlStep.Uri, exception);
		...
		{
			Logger.Error("Exception while processing pipeline for {0}, error was {1}", propertyBag.OriginalUrl, exception);
		...
		{
			Logger.Error("Download progress for step {0}", downloadProgressEventArgs.Step.Uri);
