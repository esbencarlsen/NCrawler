	...
	internal static class FilterLoader
	{
		...
		internal static FilterWrapper LoadAndInitIFilter(string fileName, string extension)
		{
			...
			if (persistFile != null)
			{
				...
				IfilterFlags flags;
				const IfilterInit iflags = IfilterInit.CanonHyphens |
					IfilterInit.CanonParagraphs |
					IfilterInit.CanonSpaces |
					IfilterInit.ApplyIndexAttributes |
					IfilterInit.HardLineBreaks |
					IfilterInit.FilterOwnedValueOk;

				if (filter.Filter.Init(iflags, 0, IntPtr.Zero, out flags) == FilterReturnCode.SOk)
				{
					return filter;
				}
			}
